...
class AppState extends ChangeNotifier {
  ...
  void renameCategory(String oldName, String newName) {
    ...
    saveData(); notifyListeners();
  }

  void renameSubCategory(String cat, String oldSub, String newSub) {
    for (var q in _questions) {
      if (q.category == cat && q.subCategory == oldSub) q.subCategory = newSub;
    }
    saveData(); notifyListeners();
  }
  ...
  void moveQuestion(String qId, String newCat, String newSubCat) {
    ...
    if (idx != -1) {
      _questions[idx].category = newCat;
      _questions[idx].subCategory = newSubCat;
      saveData(); notifyListeners();
    }
  }
...
class _MainNavigationScreenState extends State<MainNavigationScreen> {
  int _currentIndex = 0;
  final List<Widget> _screens = const[HomeScreen(), OrganizeScreen(), ImportScreen(), AnalysisBaseScreen()];
...
// ==========================================
// EXAM SCREEN (Swipe added via PageView)
// ==========================================
                  ...
                  Row(
                    ...
                    children: [
                      OutlinedButton(onPressed: () { 
                        ...
                        setState(() { _answers.remove(qId); _statuses[qId] = QuestionStatus.notAnswered.index; }); 
                        _saveState(); 
                      }, child: const Text('CLEAR')),
                      OutlinedButton(onPressed: () {
                        final qId = widget.questions[_currentIndex].id;
                        _updateStatusAndNext(_answers.containsKey(qId) ? QuestionStatus.answeredAndMarked : QuestionStatus.markedForReview);
                    ...
                    children:[
                      ...
                      if (_currentIndex < widget.questions.length - 1)
                        FilledButton(onPressed: () {
                           final qId = widget.questions[_currentIndex].id;
                           _updateStatusAndNext(_answers.containsKey(qId) ? QuestionStatus.answered : QuestionStatus.notAnswered);
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          bottom: TabBar(
            indicator: BoxDecoration(color: inkBlack),
            labelColor: paperBg,
            unselectedLabelColor: inkBlack,
...
class GlobalStatsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    sessionsByCategory.forEach((cat, list) {
      ...
      categoryStats.add({'cat': cat, 'acc': accuracy, 'avgTime': avgTime});
    });
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children:[
          const Text('ACCURACY_GRAPH [%]', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 24),
          Container(
            height: 250,
            padding: const EdgeInsets.all(16),
            ...
            child: BarChart(BarChartData(
              ...
              gridData: FlGridData(show: true, drawVerticalLine: false, getDrawingHorizontalLine: (val) => FlLine(color: Colors.black26, strokeWidth: 1, dashArray: [4, 4])),
              borderData: FlBorderData(show: false),
              barGroups: barGroups,
            )),
          ),
          const SizedBox(height: 32),
          const Text('CATEGORY_BREAKDOWN', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 16),
          ...categoryStats.map((stat) => Card(
            child: Padding(
              padding: const EdgeInsets.all(16.0),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(stat['cat'].toUpperCase(), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                  const Divider(),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children:[
                      Text('ACCURACY: ${(stat['acc'] as double).toStringAsFixed(1)}%'),
                      Text('AVG TIME/Q: ${(stat['avgTime'] as double).toStringAsFixed(1)}s'),
                    ],
                  )
                ],
              ),
...
// ==========================================
// ORGANIZE SCREEN (Rearrangement added)
// ==========================================
class OrganizeScreen extends StatelessWidget {
  ...
  void _renameCatDialog(BuildContext context, String oldName) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('RENAME_DIR'),
        content: TextField(controller: ctrl),
        actions:[
          OutlinedButton(onPressed: () => Navigator.pop(ctx), child: const Text('CANCEL')),
          FilledButton(onPressed: () {
            if(ctrl.text.trim().isNotEmpty) context.read<AppState>().renameCategory(oldName, ctrl.text.trim());
            Navigator.pop(ctx);
  ...
  void _renameSubCatDialog(BuildContext context, String cat, String oldSub) {
    ...
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('RENAME_SUB_DIR'),
        content: TextField(controller: ctrl),
        actions:[
          OutlinedButton(onPressed: () => Navigator.pop(ctx), child: const Text('CANCEL')),
          FilledButton(onPressed: () {
            if(ctrl.text.trim().isNotEmpty) context.read<AppState>().renameSubCategory(cat, oldSub, ctrl.text.trim());
            Navigator.pop(ctx);
          }, child: const Text('CONFIRM')),
        ],
      )
  ...
  void _moveQuestionDialog(BuildContext context, Question q) {
    final catCtrl = TextEditingController(text: q.category);
    final subCtrl = TextEditingController(text: q.subCategory);
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children:[
            TextField(controller: catCtrl, decoration: const InputDecoration(labelText: 'DEST_DIR')),
            const SizedBox(height: 16),
            TextField(controller: subCtrl, decoration: const InputDecoration(labelText: 'DEST_SUB_DIR')),
          ],
        ),
        actions:[
          OutlinedButton(onPressed: () => Navigator.pop(ctx), child: const Text('CANCEL')),
          FilledButton(onPressed: () {
            if(catCtrl.text.trim().isNotEmpty && subCtrl.text.trim().isNotEmpty) {
              context.read<AppState>().moveQuestion(q.id, catCtrl.text.trim(), subCtrl.text.trim());
            }
            Navigator.pop(ctx);
